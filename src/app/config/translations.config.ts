// src/app/config/translations.config.ts

export const TRANSLATIONS = {
  activity: {
    en: 'Activity',
    de: 'Aktivität',
    fr: 'Activité',
    es: 'Actividad',
    it: 'Attività',
    hu: 'Aktivitással',
    zh: '活动',
  },
  activityTitle: {
    de: 'Mit dieser Aktivität:',
    fr: 'Ayant cette activité:',
    es: 'Con esta actividad:',
    it: 'Con questa attività:',
    hu: 'Ezzel az aktivitással:',
    en: 'With this activity:',
    zh: '具有此活动：',
  },
  addressTitle: {
    de: 'An dieser Adresse wohnhaft:',
    fr: 'Domicilié.e.s à cette adresse:',
    es: 'Residente en esta dirección:',
    it: 'Residente a questo indirizzo:',
    hu: 'Ezen a címen lakó személyd:',
    en: 'Residing at this address:',
    zh: '居住在此地址：',
  },
  advanced_search: {
    de: 'erweiterte Suche',
    fr: 'recherche avancée',
    es: 'búsqueda avanzada',
    it: 'ricerca avanzata',
    en: 'advanced search',
    hu: 'összetett keresés',
    zh: '高级搜索',
  },
  projectsInput: {
    de: 'Projekt',
    fr: 'Projet',
    es: 'Proyecto',
    it: 'Progetto',
    hu: 'Projekt',
    en: 'Project',
    zh: '项目',
  },
  biblioHU: {
    de: 'Bibliografie Harmonia Universalis',
    fr: 'Bibliographie Harmonia Universalis',
    es: 'Bibliografía Harmonia Universalis',
    it: 'Bibliografia Harmonia Universalis',
    en: 'Bibliography Harmonia Universalis',
    hu: 'Harmonia Universalis bibliográfia',
    zh: 'Harmonia Universalis 书目',
  },
  buildingTitle: {
    de: 'Gebäude und Denkmäler:',
    fr: 'Edifices et monuments:',
    es: 'Edificios y monumentos:',
    it: 'Edifici e monumenti:',
    hu: 'Épületek és emlékmű:',
    en: 'Buildings and monuments:',
    zh: '建筑和纪念碑：',
  },
  classesListTitle: {
    de: 'SuperKlassen:',
    fr: 'Superclasses:',
    es: 'Superclases:',
    it: 'Superclassi:',
    hu: 'Szuperosztályok:',
    en: 'Superclasses:',
    zh: '超类：',
  },
  clickToDisplay: {
    de: 'Klicken Sie zum Anzeigen',
    fr: 'cliquez pour afficher',
    es: 'haga clic pour mostrar',
    hu: 'kattintson a megtekintéshezr',
    it: 'fare clic per visualizzare',
    en: 'click to display',
    zh: '点击显示',
  },
  clickToDownload: {
    de: 'Klicken Sie zum Download',
    fr: 'cliquez pour télécharger',
    es: 'haga clic pour descargarlo',
    hu: 'kattintson a letöltéshez',
    it: 'fare clic per scaricare',
    en: 'click to download',
    zh: '点击下载',
  },
  contextTitle: {
    de: 'In diesem Kontext anwesend:',
    fr: 'Présents dans ce contexte:',
    es: 'Presentes en este contexto:',
    it: 'Presente in questo contesto:',
    hu: 'Ebben az összefüggésben jelen van:',
    en: 'Present in this context:',
    zh: '在此情境中出现：',
  },
  currentAddress: {
    de: 'Aktuelle Adresse:',
    fr: 'Adresse actuelle :',
    es: 'Dirección actual:',
    it: 'Indirizzo attuale:',
    hu: 'Jelenlegi cím:',
    en: 'Current address:',
    zh: '当前地址：',
  },
  fetchAddress: {
    de: 'Adresse abrufen',
    fr: 'Récupérer l\'adresse',
    es: 'Obtener dirección',
    it: 'Recupera indirizzo',
    hu: 'Cím lekérése',
    en: 'Fetch address',
    zh: '获取地址',
  },
  fetchingAddress: {
    de: 'Adresse wird abgerufen...',
    fr: 'Récupération de l\'adresse…',
    es: 'Obteniendo dirección...',
    it: 'Recupero indirizzo...',
    hu: 'Cím lekérése...',
    en: 'Fetching address...',
    zh: '正在获取地址...',
  },
  dateHeader: {
    de: 'Datum',
    fr: 'Date',
    es: 'Fecha',
    hu: 'Dátuma',
    it: 'Data',
    en: 'Date',
    zh: '日期',
  },
  externalLinksTitle: {
    de: 'Externe Links',
    fr: 'Liens externes',
    es: 'Enlaces externos',
    hu: 'Külső hivatkozások',
    it: 'Collegamenti esterni',
    en: 'external links',
    zh: '外部链接',
  },
  factGridQuery: {
    de: 'FactGrid Abfrage',
    fr: 'Requête FactGrid',
    es: 'Consulta FactGrid',
    hu: 'FactGrid lekérdezés',
    it: 'Interrogazione FactGrid',
    en: 'FactGrid query',
    zh: 'FactGrid 查询',
  },
  familyNameTitle: {
    de: 'Mit diesem Familiennamen:',
    fr: 'Portant ce nom de famille:',
    es: 'Llevando este apellido:',
    it: 'Portando questo cognome:',
    hu: 'Ezt a vezetéknevet viselve:',
    en: 'With this family name:',
    zh: '拥有此姓氏：',
  },
  fields: {
    de: 'Forschungsfelder',
    fr: 'domaines de recherche',
    es: 'campos de investigación',
    it: 'aree di ricerca',
    en: 'fields of research',
    hu: 'kutatási területek',
    zh: '研究领域',
  },
  filterPeopleActivate: {
    de: 'Personen filtern',
    fr: 'Filtrer les personnes',
    es: 'Filtrar personas',
    it: 'Filtra le persone',
    hu: 'Személyek szűrése',
    en: 'Filter people',
    zh: '筛选人员',
  },
  filterPeopleDeactivate: {
    de: 'Personen filtern deaktivieren',
    fr: 'Désactiver le filtrage des personnes',
    es: 'Desactivar filtro de personas',
    it: 'Disattiva filtro persone',
    hu: 'Személyek szűrésének kikapcsolása',
    en: 'Deactivate people filter',
    zh: '停用人员筛选',
  },
  filterPublicationActivate: {
    de: 'Publikationen filtern',
    fr: 'Filtrer les publications',
    es: 'Filtrar publicaciones',
    it: 'Filtra le pubblicazioni',
    hu: 'Publikációk szűrése',
    en: 'Filter publications',
    zh: '筛选出版物',
  },
  filterPublicationDeactivate: {
    de: 'Publikationen filtern deaktivieren',
    fr: 'Désactiver le filtrage des publications',
    es: 'Desactivar filtro de publicaciones',
    it: 'Disattiva filtro pubblicazioni',
    hu: 'Publikációk szűrésének kikapcsolása',
    en: 'Deactivate publications filter',
    zh: '停用出版物筛选',
  },
  filterResults: {
    de: 'Ergebnisse filtern',
    fr: 'Filtrer les résultats',
    es: 'Filtrar resultados',
    it: 'Filtra i risultati',
    en: 'Filter results',
    hu: 'Eredmények szűrése',
    zh: '筛选结果',
  },
  formerVisitsTitle: {
    de: 'Sie haben besucht:',
    fr: 'vous avez visité :',
    es: 'has visitado :',
    it: 'hai visitato :',
    en: 'You have visited:',
    hu: 'Ön meglátogatta:',
    zh: '您访问过：',
  },
  instancesListTitle: {
    de: 'Instanzen (Limit: 200):',
    fr: 'Instances (limite: 200):',
    es: 'Entidades (limite: 200):',
    it: 'Entità (limite: 200):',
    hu: 'Entitások (korlát: 200):',
    en: 'Instances (limit: 200):',
    zh: '实例（上限：200）：',
  },
  instancesListTitle_50: {
    de: 'Instanzen (Limit: 50):',
    fr: 'Instances (limite: 50):',
    es: 'Entidades (limite: 50):',
    it: 'Entità (limite: 50):',
    hu: 'Entitások (korlát: 50):',
    en: 'Instances (limit: 50):',
    zh: '实例（上限：50）：',
  },
  itemsInput: {
    de: 'Items suchen',
    fr: 'Items',
    es: 'Buscar items',
    it: 'Cerca gli itemi',
    hu: 'Keresés az elemek között',
    en: 'Search items',
    zh: '搜索项目',
  },
  linkedPagesTitle: {
    de: 'Verlinkte Seiten',
    fr: 'Pages liées',
    es: 'Páginas enlazadas',
    hu: 'Kapcsolódó oldalak',
    it: 'Pagine collegate',
    en: 'Linked pages',
    zh: '相关页面',
  },
  listTitle: {
    de: 'Liste',
    fr: 'Liste',
    es: 'Lista',
    it: 'Lista',
    hu: 'Lista',
    en: 'List',
    zh: '列表',
  },
  locationHeader: {
    de: 'Ort',
    fr: 'Lieu',
    es: 'Lugar',
    hu: 'Közzététel helye',
    it: 'Luogo',
    en: 'Location',
    zh: '地点',
  },
  mainPage: {
    de: 'HauptSeite',
    fr: 'page principale',
    es: 'página principal',
    hu: 'főoldal',
    it: 'pagina principale',
    en: 'main page',
    zh: '主页',
  },
  natureOfListTitle: {
    de: 'Instanz von ',
    fr: 'Instance de ',
    es: 'Instancia de ',
    it: 'Istanza de ',
    hu: 'Példánya a ',
    en: 'Instance of ',
    zh: '属于：',
  },
  newSearch: {
    de: 'neue Suche',
    fr: 'nouvelle recherche',
    es: 'nueva búsqueda',
    hu: 'új keresés',
    it: 'nuova ricerca',
    en: 'new search',
    zh: '新搜索',
  },
  noneLabel: {
    en: 'none',
    fr: 'aucune',
    de: 'keine',
    es: 'ninguno',
    it: 'nessuno',
    hu: 'nincs',
    zh: '无',
  },
  organisationTitle: {
    de: 'Mitglieder:',
    fr: 'Membres:',
    es: 'Membres:',
    it: 'Membri:',
    hu: 'Tagok:',
    en: 'Members:',
    zh: '成员：',
  },
  patientsTitle: {
    de: 'Patienten/innen',
    fr: 'Patient.e.s',
    es: 'Pacientes',
    it: 'Pazienti',
    hu: 'Páciensek',
    en: 'Patients',
    zh: '病人',
  },
  prefix1: {
    de: 'Klassenhierarchie: abhängige Klasse von ',
    fr: 'hiérarchie des classes: classe dépendante de ',
    es: 'jerarquía de clases: clase dependiente de ',
    it: 'gerarchia delle classe: classe dipendente da ',
    hu: 'osztályhierarchia: osztály függő ',
    en: 'Class hierarchy: dependent class of ',
    zh: '类层次结构：依赖类 ',
  },
  prefix2: {
    de: 'Klassenhierarchie: Klasse mit',
    fr: 'hiérarchie des classes: classe dotée de ',
    es: 'jerarquía de clases: clase con ',
    it: 'gerarchia delle classe: classe con ',
    hu: 'osztályhierarchia: osztály a ',
    en: 'Class hierarchy: class with ',
    zh: '类层次结构：具有 ',
  },
  projects: {
    de: 'Forschungsprojekten',
    fr: 'projets de recherche',
    es: 'proyectos de investigación',
    it: 'progetti di ricerca',
    en: 'research projects',
    hu: 'kutatási projektek',
    zh: '研究项目',
  },

  projectSearch: {
    de: 'Suche einen Projekt',
    fr: 'Rechercher un projet',
    es: 'Buscar un proyecto',
    it: 'Cerca un progetto',
    en: 'Search a project',
    hu: 'Projekt keresése',
    zh: '搜索项目',
  },
  projectOverlayFilterLabel: {
    de: 'Projekte filtern',
    fr: 'Filtrer les projets',
    es: 'Filtrar proyectos',
    it: 'Filtrare i progetti',
    en: 'Filter projects',
    hu: 'Projektek szűrése',
    zh: '筛选项目',
  },
  projectOverlayFilterPlaceholder: {
    de: 'Eingeben zum Filtern...',
    fr: 'Entrer pour filtrer...',
    es: 'Introduzca para filtrar...',
    it: 'Inserire per filtrare...',
    en: 'Enter to filter...',
    hu: 'Írjon be a szűréshez...',
    zh: '输入以筛选...',
  },
  projectOverlayEmpty: {
    en: 'No projects found',
    fr: 'Aucun projet trouvé',
    de: 'Keine Projekte gefunden',
    es: 'No se encontraron proyectos',
    it: 'Nessun progetto trovato',
    hu: 'Nincs találat',
    zh: '未找到项目',
  },
  seeMore: {
    en: 'See __n__ more results',
    fr: 'Voir __n__ autres résultats',
    de: 'Siehe __n__ weitere Ergebnisse',
    es: 'Ver __n__ más resultados',
    it: 'Vedi __n__ altri risultati',
    hu: 'Lásd még __n__ eredményt',
    zh: '查看更多 __n__ 结果',
  },

  projectName: {
    de: 'Projektname',
    fr: 'Nom du projet',
    es: 'Nombre del proyecto',
    it: 'Nome del progetto',
    hu: 'Projekt neve',
    en: 'Project name',
    zh: '项目名称',
  },

  pupilTitle: {
    de: 'Schüler/innen',
    fr: 'Élèves et disciples',
    es: 'Alumnos y discípulos',
    it: 'Studenti e discepoli',
    hu: 'Müvek',
    en: 'Pupils and disciples',
    zh: '学生和弟子',
  },
  results: {
    de: 'Ergebnisse',
    fr: 'Résultats',
    es: 'Resultados',
    it: 'Risultati',
    hu: 'Eredmények',
    en: 'Results',
    zh: '结果',
  },
  setTitle: {
    de: 'Erhalten:',
    fr: 'Inclus :',
    es: 'Incluye:',
    it: 'Include:',
    hu: 'Tartalmazza:',
    en: 'Included:',
    zh: '包含：',
  },
  stemma: {
    de: 'Stemma_aufwärts',
    fr: 'Précédent_dans_le_stemma',
    es: 'Precedente_en_el_stemma',
    hu: 'Preceding_in_stemma',
    it: 'Precedente_in_stemma',
    en: 'Previous in stemma',
    zh: '谱系上一个',
  },
  subInfoTitle: {
    de: 'Q-Item Information',
    fr: "Information sur l'élément",
    es: 'Información sobre el elemento',
    it: "Informazioni sull'elemento",
    hu: 'Az elemre vonatkozó információk',
    en: 'Q-Item Information',
    zh: 'Q项目信息',
  },
  subclassesListTitle: {
    de: 'Unterklassen:',
    fr: 'Sous-classes:',
    es: 'Subclases:',
    hu: 'Alosztályok:',
    it: 'Sottoclassi:',
    en: 'Subclasses:',
    zh: '子类：',
  },
  subtitle: {
    de: 'eine Datenbank für Historiker*innen',
    fr: 'une base de données pour historien.nes',
    es: 'una base de datos para historiadores',
    it: 'un database per gli storici',
    hu: 'egy adatbázis történészeknek',
    en: 'a database for historians',
    zh: '历史学家的数据库',
  },
  suffix1: {
    de: 'Klassen:',
    fr: 'classes:',
    es: 'clases:',
    it: 'classi:',
    hu: 'osztályok:',
    en: 'classes:',
    zh: '类：',
  },
  titleHeader: {
    de: 'Titel',
    fr: 'Titre',
    es: 'Título',
    hu: 'Cím',
    it: 'Titolo',
    en: 'Title',
    zh: '标题',
  },
  workTitle: {
    de: 'Werke',
    fr: 'Œuvres',
    es: 'Obras',
    it: 'Opere',
    hu: 'Müvek',
    en: 'Works',
    zh: '作品',
  },
  // keys used by FactgridSubtitlesService (centralize subtitles/translations here)
  subtitle_members: {
    en: 'List of members',
    de: 'Mitgliederverzeichnis',
    fr: 'Liste des membres',
    es: 'Lista de miembros',
    it: 'Elenco dei membri',
    hu: 'A tagok listája',
    zh: '成员列表',
  },
  subtitle_further: {
    en: 'further',
    de: 'weiteren',
    fr: 'autres',
    es: 'más',
    it: 'di più',
    hu: 'további',
    zh: '更多',
  },
  subtitle_sources: {
    en: 'Sources',
    de: 'Quellen',
    fr: 'Sources',
    es: 'Fuentes',
    it: 'Fonti',
    hu: 'Források',
    zh: '来源',
  },
  subtitle_life_and_family: {
    en: 'Life and family',
    de: 'Leben und Familie',
    fr: 'Vie et famille',
    es: 'Vida y familia',
    it: 'Vita e famiglia',
    hu: 'Élet és család',
    zh: '生活与家庭',
  },
  subtitle_education: {
    en: 'Education',
    de: 'Ausbildung',
    fr: 'Éducation',
    es: 'Educación',
    it: 'Educazione',
    hu: 'Oktatás',
    zh: '教育',
  },
  subtitle_career_and_activities: {
    en: 'Career and activities',
    de: 'Beruf und Aktivitäten',
    fr: 'Carrière et activités',
    es: 'Carrera y actividades',
    it: 'Carriera e attività',
    hu: 'Karrier és tevékenységek',
    zh: '职业与活动',
  },
  subtitle_sociability_and_culture: {
    en: 'Sociability and culture',
    de: 'Soziabilität und Kultur',
    fr: 'Sociabilité et culture',
    es: 'Sociabilidad y cultura',
    it: 'Sociabilità e cultura',
    hu: 'Szociabilitás és kultúra',
    zh: '社交与文化',
  },
  subtitle_place: {
    en: 'Place',
    de: 'Ort',
    fr: 'Lieu',
    es: 'Lugar',
    it: 'Luogo',
    hu: 'Hely',
    zh: '地点',
  },
  subtitle_event: {
    en: 'Event',
    de: 'Ereignis',
    fr: 'Évènement',
    es: 'Evento',
    it: 'Evento',
    hu: 'Esemény',
    zh: '事件',
  },
  // Generic small UI strings
  no_results: {
    en: 'No results',
    de: 'Keine Ergebnisse',
    fr: 'Aucun résultat',
    es: 'Sin resultados',
    it: 'Nessun risultato',
    hu: 'Nincsenek találatok',
    zh: '无结果',
  },
  subtitle_document: {
    en: 'Document',
    de: 'Dokument',
    fr: 'Document',
    es: 'Documento',
    it: 'Documento',
    hu: 'Dokumentum',
    zh: '文档',
  },
  subtitle_database: {
    en: 'Database',
    de: 'Datenbank',
    fr: 'Base de données',
    es: 'Base de datos',
    it: 'Base dati',
    hu: 'Adatbázis',
    zh: '数据库',
  },
};
