...
export class HarmoniaUniversalisComponent implements OnInit, AfterViewInit {
  ...
  constructor() {
    this.dataSource.sortingDataAccessor = (data, sortHeaderId) => {
      return sortingAccessor.nestedCaseInsensitive(data, sortHeaderId);
    };
    this.dataSource.filterPredicate = (data, filter: string) => {
      return (
        data.author.label.toLocaleLowerCase().includes(filter) ||
  ...
  ngOnInit() {
    ...
    this.dataSource$ = combineLatest([this.behavior$, v$]).pipe(
      map((res) => {
  ...
  onClick(query) {
    //handling click for downlooding the filtered data
    let u = query;
