...
export class HuDatabaseService {
  ...
  sparqlBuilding(lang) {
    ...
    let u =
      'https://database.factgrid.de/query/#%23biblio%0ASELECT%20DISTINCT%20%3Ftitle%20%3FtitleLabel%20%3Fauthor%20%3FauthorLabel%20%3Fdate%20%3Flocation%20%3FlocationLabel%20%3F' +
      ...
      'author%3B%0A%20%20%20%20%20%20%20%20wdt%3AP222%20%3Fdate%3B%0A%20%20%20%20%20%20%20%20wdt%3AP241%20%3Flocation%20.%0A%20%20%3Fcountry%20%5Ewdt%3AP297%20%3Flocation%3B%0A%20%20%20%20%20%20%20%20%20%20wdt%3AP2%20%20%20%3Fc%20%20' +
      lang;
    return u;
  }

  listFromSparql(res) {
    // create the qid in the results.bindings
    if (res !== undefined) {
      if (res.results !== undefined) {
        for (let i = 0; i < res.results.bindings.length; i++) {
          ...
          res.results.bindings[i]['location'].id = res.results.bindings[i][
            'location'
          ].value.replace('https://database.factgrid.de/entity/', '');
          res.results.bindings[i]['location'].label =
            res.results.bindings[i]['locationLabel'].value;
