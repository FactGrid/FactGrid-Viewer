...
export class ParisSearchComponent implements OnInit, OnDestroy {
  ...
  selectedParisItemsList: any[] = (JSON.parse(localStorage.getItem('selectedItems')) || []).filter(
    (item) => item && item.value && item.value.id
  );
