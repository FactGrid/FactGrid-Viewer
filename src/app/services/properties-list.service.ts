...
export class PropertiesListService {
  ...
  propertiesList2Building(id) {
    let prefix =
      'https://database.factgrid.de/query/#SELECT%20DISTINCT%20%3Fitem%20%3FitemLabel%20WHERE%20%7B';
  ...
  get qualifierPropertiesListBuilding() {
    let prefix =
      'https://database.factgrid.de/query/#SELECT%20DISTINCT%20%3Fitem%20%3FitemLabel%20%3FpropertyType%20%20WHERE%20%7B%20';
    let suffix =
      '%20VALUES%20%3Fqualifier%20%7B%20wd%3AQ514317%20wd%3AQ21403%20%7D%0A%20%3Fitem%20wikibase%3ApropertyType%20%3FpropertyType%3B%0A%20%20%20%20%20%20%20%20%20%20%20wdt%3AP8%20%3Fqualifier.%0A%7D%0AORDER%20BY%20%3FitemLabel';
    let v: any[];
    ...
    return this.request.getList(u).pipe(
      map((res) => this.listFromSparql(res)),
      map((res) => this.transformListFromSparql(res))
    );
  }
  ...
  deepNatureOf() {
    ...
    if (this.lang.selectedLang === 'fr') {
      u = "Nature de l'élément (recherche approfondie)";
    }
    if (this.lang.selectedLang === 'es') {
      u = 'Instancia de (investigación en profundidad)';
    }
    if (this.lang.selectedLang === 'it') {
      u = 'È (ricerca approfondita)';
    }
    return u;
  }
  ...
  changeList(data) {
    if (data !== undefined) {
      for (let i = 0; i < data.length; i++) {
        let type = this.removeSubstring(
          data[i]['propertyType'].value,
