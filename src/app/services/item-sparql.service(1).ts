...
export class ItemSparqlService {
  ...
  itemSparql(item) {
    // get a list depending of the type of the item
    let a: boolean = false;
    ...
    this.superclassTest = this.sparqlAsk(this.keywordTest(item.id, 'Q945280')).pipe(
      startWith(false)
    ); // boolean test for item as a superclass (Q945280)
    this.superclass1Test = this.sparqlAsk(this.keywordTest(item.id, 'Q960698')).pipe(
      startWith(false)
    ); // boolean test for item as a superclass (1st-order subclasses) (Q945280)
    ...
    ); // data for the component sparql0: all subclasses of a superclass
    ...
    this.sparql1$ = forkJoin([
      ...
      this.Q16200Test,
      this.Q77457Test,
    ]) //data for the component sparql1 : organisations, career statements, creators, family names, addressses, classes of FactGrid properties
      .pipe(
    ...
    this.sparql2$ = forkJoin([this.Q140759Test, this.Q16200Test]).pipe(
      ...
      startWith([undefined, undefined])
    ); // data for the component sparql2 : health pratictioners, current address

    this.sparql3$ = forkJoin([this.masterTest, this.listTest, this.setTest, this.Q16200Test]).pipe(
      switchMap(([test1, test2, test3, test4]) =>
        this.selectSparql3(test1, test2, test3, test4, item)
      ),
      startWith([undefined, undefined])
    ); // data for the component sparql3 : pupils and students, list of items of a class, parts of a class

    this.sparql4$ = forkJoin([this.Q8Test, this.GOVTest]).pipe(
      switchMap(([test1, test2]) => this.selectSparql4(test1, test2, item)),
      startWith([undefined, undefined])
    ); // data for the component sparql4: locality (buildings, etc.)
  ...
  selectSparql0(test1, test2, item) {
    // select the right result for sparql0
  ...
  selectSparql1(test1, test2, test3, test4, test5, test6, item) {
    // select the right result for the component sparql1
    let result: Observable<any[]>;
    if (test5 === true) {
      // addresses
        ...
        if (test2 === true) {
          ...
          if (test3 === true) {
            ...
            if (test4 === true) {
              ...
              if (test6 === true) {
                // properties of a class of Factgrid properties
  ...
  selectSparql2(test1, test2, item) {
    // select the right result for sparql2
    let result: Observable<any[]>;
    if (test1 === true) {
      result = this.Q140759Sparql(test1, item); // health practitioners
    } else {
      if (test2 === true) {
        // current address for an old address
  ...
  selectSparql3(test1, test2, test3, test4, item) {
    // select the right result for sparql3
    let result: Observable<any[]>;
    if (test1 === true) {
  ...
  Q24499TestGet(item) {
    // to get the boolean test for Q2449 (family name)
  ...
  Q8TestGet(item) {
    // to get the boolean test for Q8 (locality); deprecated
    let test: boolean = false;
  ...
  Q172192TestGet(item) {
    // to get the boolean test for Q172192 (list); deprecated
    let test: boolean = false;
  ...
  Q77457TestGet(item) {
    // to get the boolean test for Q77457 (class of FactGrid properties)
    let test: boolean = false;
    if (
      item &&
      item.claims &&
  ...
  Q140759Sparql(test, res) {
    //  // query health practioners: the list of their patients
    if (test === true) {
      let prefix =
  ...
  masterSparql(test, res) {
    //  // query master: the list of their students and disciplines
    if (test === true) {
      let prefix =
  ...
  superclass1Sparql(test, res) {
    // query 1st-order subclasses of item
    if (test === true) {
      let prefix =
  ...
  Q24499Sparql(res) {
    ...
    return this.sparqlQuery(u).pipe(
      map((res) => ['Q24499', this.listFromSparql(res).results.bindings])
    );
  }
  ...
  Q77457Sparql(res) {
    //  // query class of FactGrid properties: the list of the properties belonging to the class
    let prefix =
  ...
  sparqlAsk(sparql) {
    // sparql query for a boolean test;
    let u = '';
  ...
  subclassTest(a, type) {
    // to test to check whether the class of an item is the subclass of a given superclass (in the hierarchy of classes)
  ...
  classTest(a, type) {
    // to test to check whether t item is the subclass of a given superclass (in the hierarchy of classes)
    //  let u = "https://database.factgrid.de/query/#ASK%20%7Bwd%3A" + a + "%20wdt%3AP3%2a%20wd%3A" + type + ".%7D%0A%20";
    let u =
      'https://database.factgrid.de/query/#ASK%20%7Bwd%3A' +
      a +
  ...
  keywordTest(a, type) {
    // to test to check whether the item is the subclass of a given superclass (in the hierarchy of classes) or of category OhdAB
    let u =
  ...
  masterSubclassTest(a) {
    // to check whether an item belongs to a subclass of painter/health care practioners/professors
    let u =
  ...
  activitiesTest(item) {
    // extract boolean test out of the array of activities for the right activity, used in
  ...
  testArrayGet(res) {
    // get the boolean test in the array of boolean tests, used in activitiesTest
    let result;
  ...
  newSparqlAdress(address: string): string {
    // to create the right address for sparql query
  ...
  listFromSparql(res) {
    // to transform the sparql response into a list to display (used in sparqlQuery())
    if (res !== undefined) {
  ...
  noResult() {
    // query in case there is no test
    let u = '';
