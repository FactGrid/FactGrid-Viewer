...
// TranslateService removed — not used by this pipe
...
export class CustomDatePipe implements PipeTransform {
  //transform FactGrid-date format into ISO-date format in order to use Angular Date Pipe
  ...
  transform(value: any): any {
    if (value !== undefined) {
      if (
        value.slice(value.length - 13) !== '-00T00:00:00Z' ||
