  ...
  implements OnInit, OnDestroy, AfterViewInit, AfterContentInit
{
  ...
  dynamicInfoList: BehaviorSubject<{ itemTypes: number[]; itemValues: number[] }[]> =
    new BehaviorSubject([]);
  ...
  get lastStatementIndex(): number {
    return this.statements.length > 0 ? this.statements.length - 1 : 0;
  }
  ...
  addStatements() {
    ...
    const initialDynamicInfo = {
      itemTypes: Array(ITEMTYPES.length).fill(0),
      itemValues: Array(ITEMTYPES.length).fill(0),
    };
  ...
  ngOnInit(): void {
    ...
    this.dynamicInfoList.pipe(takeUntil(this._onDestroy)).subscribe(() => {
    ...
    var propertyValueChanges: Observable<any> = this.query
      .get('statements')
      .valueChanges.pipe(map((res) => res[0]['property']));

    var valueValueChanges: Observable<any> = this.query
      .get('statements')
      .valueChanges.pipe(map((res) => res[0]['itemValue']));
  }
  ...
  private addOrderToItemValues(itemValues: any[]): any[] {
    ...
    return itemValues.map((itemValue, itemIndex) => {
      if (
        itemValue &&
        dynamicInfoListValue[this.lastStatementIndex].itemValues[itemIndex] !== undefined
      ) {
  ...
  private updateDropdownOptions() {
    ...
    const updatedItemTypes = ITEMTYPES.map((itemType, itemIndex) => {
      if (
        itemType &&
        dynamicInfoListValue[this.lastStatementIndex].itemTypes[itemIndex] !== undefined
      ) {
    ...
    dynamicInfoListValue[this.lastStatementIndex].itemTypes = updatedItemTypes
      .map((item) => item.order)
  ...
  protected filterItemTypes() {
    ...
    const selectedValues = this.statements.controls.map(
      (statement) => statement.get('itemType').value
    ...
    this.data.$propertiesList.subscribe((res) => {
      ...
      if (!search) {
        ...
        this.filteredPropertyMulti.next(
          this.propertiesToSelect.filter(
      ...
      this.itemValueFilterCtrl.valueChanges // moteur de recherche
        .pipe(
          ...
          map((re) => {
            ...
            return this.addOrderToItemValues([...ITEMTYPES, ...this.entityValues]).filter((value) =>
              value.label.toLowerCase().includes(search)
            );
          })
        )
