...
export class AdvancedSearchComponent implements OnInit, OnDestroy {
  ...
  formData: Data[] = [
    {
      statement: 0,
      types: { options: ITEMTYPES, selections: [] },
      values: { options: ITEMTYPES, selections: [] },
    },
  ];
  ...
  propertyDatatype(u) {
    //
    this.literalVariables = this.literals.filter(
      (literal) => (literal.type === u[1] || literal.type === 'Any') && literal.col === u[0]
    );
  ...
  qualifierPropertyDatatype(u) {
    //
    this.qualifierLiteralVariables = this.literals.filter(
      (literal) => (literal.type === u[2] || literal.type === 'Any') && literal.col === u[0]
    );
  ...
  selectedValue(value) {
    ...
    let selection1: Selection = {
      ...
      label: value[1],
      id: value[3],
    }; //TODO: value[2], which is the order of the unknown, must be updated in a function to take in account value[2] in selection1
    if (!this.statementSelection[1]) {
      this.statementSelection.push(selection1);
    } else this.statementSelection.splice(1, 1, selection1);
